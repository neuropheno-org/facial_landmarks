...
fixed = 1
model_path = 'checkpoints/inception_face_60wz_25st_AtaxiavsControl_mask_y_len20_mask_x_len10_lr1e-06_derivTrue_pntsubsetTrue_optimAdam_valBatch8_22-07-20_10-18-11'
model_name = 'inception'
net = torch.load( model_path)
if fixed:
    for param in net.parameters():
        param.requires_grad = False
net2 = nn.Sequential(*list(net.children())[:-1], nn.Linear(in_features=4*32*1, out_features=2))
...
for win_len in [100]:
    for hop in [50]:
        ...
        model_path = f"checkpoints/{net_name}_{data_input}_transferlearningfixW{fixed}_{win_len}wz_{hop}st_{g1}vs{g0}_mask_y_len{mask_y_len}_mask_x_len{mask_x_len}_lr{lr}_deriv{derivatives}_pntsubset{pose_pnts_small}_optim{optim}_valBatch{batch_val}_{time_now}"
        ...
        if save:
            fig.savefig(f"result_figs/{title}_{data_input}_transferlearningfixW{fixed}_{win_len}wz_{hop}st_{g1}vs{g0}_mask_y_len{mask_y_len}_mask_x_len{mask_x_len}_lr{lr}_deriv{derivatives}_pntsubset{pose_pnts_small}_optim{optim}_valBatch{batch_val}_{round(ttot/60)}mintime.png")
