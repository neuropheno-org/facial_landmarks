...
model_path = 'checkpoints/inception_face_100wz_50st_AtaxiavsControl_mask_y_len10_mask_x_len10_lr1e-05_derivTrue_pntsubsetTrue_optimAdam_valBatch8_22-07-12_21-53-05'
...
for win_len in [60]:
    for hop in [25]:
        ...
        y_train = [0 if "Go" in l else 1 if "La" in l else 2 if "Me" in l else -1 for l in task_ids_t]
        y_val = [0 if "Go" in l else 1 if "La" in l else 2 if "Me" in l else -1 for l in task_ids_v]
        
        # y_train = [0 if "Go" in l else 1 for l in task_ids_t]
        # y_val = [0 if "Go" in l else 1 for l in task_ids_v]
        
        val_1 = sum(y_val)
        val_0 = abs(val_1 - len(y_val))
        
        l3 = f"data train: {np.unique(y_train, return_counts=True)} "
        l4 = f"data val: {np.unique(y_val, return_counts=True)} "
        print(l3)
        print(l4)
        ...
        train_f1 = torchmetrics.F1(num_classes=len(np.unique(y_val)), average='weighted')
        train_f1.to(device)
        metrics = {k: [] for k in ["train_loss", "train_acc",
                                "val_loss", "val_acc", "train_f1", "val_f1"]}
        
        model_path = f"checkpoints/{net_name}_{data_input}_transferlearning_{win_len}wz_{hop}st_{g1}vs{g0}_mask_y_len{mask_y_len}_mask_x_len{mask_x_len}_lr{lr}_deriv{derivatives}_pntsubset{pose_pnts_small}_optim{optim}_valBatch{batch_val}_{time_now}"
        ...
        if save:
            fig.savefig(f"result_figs/{title}_{data_input}_transferlearning_{win_len}wz_{hop}st_{g1}vs{g0}_mask_y_len{mask_y_len}_mask_x_len{mask_x_len}_lr{lr}_deriv{derivatives}_pntsubset{pose_pnts_small}_optim{optim}_valBatch{batch_val}_{round(ttot/60)}mintime.png")
